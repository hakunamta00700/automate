...
from ...custom_api.models import ChatMessage
from ...custom_api.providers import BaseProvider
...
async def summarize(
    ...
    """
    AI Provider를 사용하여 대본을 요약합니다. (비동기 실행)

    대본이 토큰 제한을 초과하는 경우 자동으로 분할하여 요약합니다.

    Args:
        transcript: 요약할 대본 리스트
        model_name: 사용할 모델 이름 (기본값: 설정에서 가져옴, 일부 Provider에서만 사용)
        provider_type: 사용할 Provider 타입 (기본값: 설정에서 가져옴)
    ...
    # 토큰 제한 설정 (OpenAI 기준, 다른 Provider는 자체 제한 사용)
    ...
    final_messages = [
        ChatMessage(role="system", content="당신은 영상 대본 요약 전문가입니다."),
