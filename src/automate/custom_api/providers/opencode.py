...
class OpenCodeProvider(BaseProvider):
    ...
    def __init__(self):
        """OpenCode Provider 초기화"""
        settings = get_custom_api_settings()
        super().__init__(model_name="opencode")
    ...
    async def chat_completion(
        ...
        **kwargs,
        ...
        """OpenCode를 사용하여 Chat completion 생성
        ...
        # OpenCode 명령어 구성
        # opencode run 형식으로 실행 (stdin에서 프롬프트 읽기)
        # 프롬프트를 안전하게 전달하기 위해 stdin 사용
        command_parts = [self.command, "run"]
        command = " ".join(command_parts)

        logger.info(f"OpenCode 명령어 실행: {command}")
        ...
        try:
            ...
            if process.returncode != 0:
                error_msg = f"OpenCode 실행 실패 (반환 코드: {process.returncode})"
                if stderr_text:
                    error_msg += f"\n에러: {stderr_text}"
            ...
            logger.exception(f"OpenCode 실행 중 오류: {e}")
            raise
